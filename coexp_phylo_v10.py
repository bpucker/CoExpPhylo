...
def get_groups( annotation_file, araport_seq_file, blast_results, anno_mapping_table, huge_pep_collection, aln_folder, mincoexpseqcutoff, minseqcutoff, tmp_folder, min_intersection_cutoff, cpu, batch ):
	...
	with open( annotation_file, "w" ) as anno_out:
				...
				if coexpseqcounter >= mincoexpseqcutoff:
					...
					if len( araport_seq_file ) > 0:
						with open(anno_blast_cmd_file, "a" ) as anno_command_out:
							...
							current_anno_command = "blastp -query " + group_sequence_file + " -db " + blast_db + " -out " + blast_result_file + " -outfmt 6 -evalue 0.001 -num_threads " + str( cpu ) + "\n"
		...
		if len( araport_seq_file ) > 0:
			...
			sys.stdout.write( str( datetime.datetime.now() ) + " blast for annotation completed! " + "\n" )
			sys.stdout.flush()
...
def main( arguments ):
	...
	with open( docu_file, 'w') as f:
		...
		f.write( str ( 'config_file = ' + config_file + '\n\nPARAMETER\n--------------\n' ) )
		...
		f.write('\nTOOLS\n--------------\n')
	required_tools = [ 'nohup', 'parallel', 'blastp', 'diamond', alnmethod, treemethod ]
	...
	if mode == "batch":
		...
		batch_coexp = "nohup parallel -j " + str( int( batch/2 ) ) + " <<EOF \n"
		...
		for spec in list( config_data.keys() ):
			sys.stdout.write( str( datetime.datetime.now() ) + " - analyzing " + spec + "\n" )
			sys.stdout.flush()
			...
			with open( blast_dbs_file,  "a" ) as blast_dbs_out:
				current_blast_db_command = "makeblastdb -in " + config_data[ spec ]['pep_file'] + " -out " + blastdb + " -dbtype prot"
	...
	if mode == "batch":
		...
		sys.stdout.write( str( datetime.datetime.now() ) + "BLASTp completed! " + "\n" )
		sys.stdout.flush()
	...
	if os.path.isfile( huge_seq_collection_file ):
		...
		if not os.path.isfile( annotation_file ):
			...
			sys.stdout.flush()
			get_groups( annotation_file, araport_seq_file, blast_results, anno_mapping_table, huge_pep_collection, aln_folder, mincoexpseqcutoff, minseqcutoff, tmp_folder, min_intersection_cutoff, cpu, batch )
